...
public interface TrafficReportRepository extends JpaRepository<TrafficReport, Long> {
    @Query("SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM TrafficReport r WHERE ST_Equals(r.location, ?1) = true AND r.expiredAt > CURRENT_TIMESTAMP")
    boolean existsTrafficReportByLocationAndExpiredAt(Point point);

    @Query("SELECT r FROM TrafficReport r WHERE ST_DWithin(r.location, ?1, 10 * 0.00001) = true AND r.expiredAt > NOW() AND r.isAccept = true AND r.likeCount > -2")
